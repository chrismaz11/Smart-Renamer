...
const setLoading = (button, isLoading, loadingText = 'Processing...') => {
  if (isLoading) {
    ...
    button.innerHTML = button.dataset.originalText || 'Apply Changes'
  }
}
...
renameFilesBtn.addEventListener('click', async () => {
  if (selectedFolderPath) {
    try {
      ...
      console.error(error)
      previewPanel.innerHTML = '<div class="text-red-500">Error renaming files. See console for details.</div>'
    } finally {
